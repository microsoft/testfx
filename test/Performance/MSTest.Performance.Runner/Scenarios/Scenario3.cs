...
/// <summary>
/// Lifecycle-overhead scenario: each test class has both a <c>[TestInitialize]</c> and a
/// <c>[TestCleanup]</c> method that do minimal work (a field write), so the measured time
/// reflects the MSTest lifecycle machinery rather than test body cost.
///
/// Designed to pair with Scenario1 for a direct comparison of lifecycle overhead:
///   Scenario1 → 100 classes × 100 methods, no lifecycle hooks
///   Scenario3 → 100 classes × 100 methods, with [TestInitialize] + [TestCleanup] per method
///
/// Both scenarios run 10 000 tests to keep the execution count identical.
/// </summary>
            ...
            stringBuilder.AppendLine(
                ...
                $$"""
                  ...
                  public class UnitTest{{i}}
                  {
                      ...
                      public void Initialize() => _state = 1;

                      [TestCleanup]
                      [System.Runtime.CompilerServices.MethodImplAttribute(System.Runtime.CompilerServices.MethodImplOptions.NoInlining)]
                      public void Cleanup() => _state = 0;
