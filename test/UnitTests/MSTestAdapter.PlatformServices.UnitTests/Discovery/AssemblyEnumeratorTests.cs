...
public class AssemblyEnumeratorTests : TestContainer
{
    ...
    public void EnumerateAssemblyShouldReturnEmptyListWhenNoDeclaredTypes()
    {
        ...
        // Setup mocks
        mockAssembly.Setup(a => a.GetTypes()).Returns([]);
        _testablePlatformServiceProvider.MockFileOperations.Setup(fo => fo.LoadAssembly("DummyAssembly"))
            .Returns(mockAssembly.Object);

        AssemblyEnumerationResult result = _assemblyEnumerator.EnumerateAssembly("DummyAssembly", false);
    ...
    public void EnumerateAssemblyShouldReturnTestElementsForAType()
    {
        Mock<TestableAssembly> mockAssembly = CreateMockTestableAssembly();
        var testableAssemblyEnumerator = new TestableAssemblyEnumerator();
        var unitTestElement = new UnitTestElement(new TestMethod("DummyMethod", "DummyClass", "DummyAssembly", displayName: null));
        ...
        AssemblyEnumerationResult result = testableAssemblyEnumerator.EnumerateAssembly("DummyAssembly", false);
        _warnings.AddRange(result.Warnings);
...
internal sealed class TestableAssemblyEnumerator : AssemblyEnumerator
{
    internal TestableAssemblyEnumerator()
    {
        ...
        IReflectionOperations wrappedReflectionOperations = MockableReflectionOperations.Create(mockReflectionOperations);
        var typeValidator = new Mock<TypeValidator>(wrappedReflectionOperations);
        var testMethodValidator = new Mock<TestMethodValidator>(wrappedReflectionOperations, false);
        MockTypeEnumerator = new Mock<TypeEnumerator>(
            typeof(DummyTestClass),
            "DummyAssembly",
            wrappedReflectionOperations,
