...
// Ported from https://github.com/dotnet/runtime/blob/513ff1acc981118e4b981e965dce614c3b8770f5/src/libraries/System.IO.Hashing/tests/XxHash128Tests.cs
public class XxHash128Tests : TestContainer
{
...
#if NET
            // TestFx-specific: We don't have HashToUInt128.
...
#pragma warning restore IDE0042
            ...
            for (int ctorIteration = 0; ctorIteration < ctorIterations; ctorIteration++)
            {
                ...
                for (int trial = 0; trial < 2; trial++)
                {
                    ...
                    while (!input.IsEmpty)
                    {
                        ...
                        // TestFx-specific: We don't have Append(ReadOnlySpan<byte>). So, call .ToArray().
                        hash.Append(slice.ToArray());
    ...
    // TestFx-specific: We don't have Clone.
#if false
    public void Clone_ProducesSameSequence()
    {
        ...
        for (int outer = 0; outer < 4; outer++)
        {
            ...
            Verify(hash.HashLengthInBytes == clone.HashLengthInBytes);
            Verify(hash.GetCurrentHash() == clone.GetCurrentHash());
    ...
    {
        ulong high = BinaryPrimitives.ReadUInt64BigEndian(span);
        ulong low = BinaryPrimitives.ReadUInt64BigEndian(span.Slice(8));
    ...
    private record struct Hash128(ulong High, ulong Low)
    {
        public override readonly string ToString()
            => $"Hash128 {{ High = 0x{High:x16}, Low = 0x{Low:x16} }}";
    }

    private static IEnumerable<(ulong HashHigh, ulong HashLow, long Seed, string Ascii)> TestCases()
    {
        yield return (HashHigh: 0x99aa06d3014798d8UL, HashLow: 0x6001c324468d497fUL, Seed: 0x0000000000000000L, Ascii: string.Empty);
        yield return (HashHigh: 0x4a807558806f6b31UL, HashLow: 0xeca8475b2cc08feeUL, Seed: 0x00000000000013f0L, Ascii: string.Empty);
        yield return (HashHigh: 0xbecb6b41f8c7c702UL, HashLow: 0x8bcfeb4c4ff25443UL, Seed: 0x00000000000008baL, Ascii: string.Empty);
        yield return (HashHigh: 0xa2a900ec02fd312fUL, HashLow: 0xa75380084c7b1df2UL, Seed: 0x0000000000001bc1L, Ascii: string.Empty);
