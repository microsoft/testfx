...
public sealed class GitHubActionsSlowTestReporterTests
{
    ...
    public async Task ConsumeAsync_SupersededRetryAttempt_KeepsTrackingAsync()
    {
        ...
        // A superseded retry attempt (a later attempt for the same test node exists) is terminal-shaped
        // (FailedTestNodeStateProperty) but must NOT stop tracking: the test as a whole is still running
        // its next attempt, so the slow-test notice must still surface past the threshold.
        await reporter.ConsumeAsync(null!, CreateMessage("u1", "Ns.T1", new FailedTestNodeStateProperty(), retryAttempt: new RetryAttemptProperty(1, isSuperseded: true)), CancellationToken.None).ConfigureAwait(false);
    ...
    private static TestNodeUpdateMessage CreateMessage(string uid, string fullyQualifiedName, IProperty property, string? displayName = null, RetryAttemptProperty? retryAttempt = null)
    {
