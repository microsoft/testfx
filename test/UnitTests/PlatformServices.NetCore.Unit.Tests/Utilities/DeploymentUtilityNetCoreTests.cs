...
namespace MSTestAdapter.PlatformServices.Tests.Utilities
{
    ...
    public class DeploymentUtilityTests
    {
        ...
        public void DeployShouldDeployPdbWithSourceIfPdbFileIsPresentInSourceDirectory()
        {
            ...
            // Assert.
            var sourceFile = Assembly.GetEntryAssembly().GetName().Name + ".dll";
            var pdbFile = Assembly.GetEntryAssembly().GetName().Name + ".pdb";
            this.mockFileUtility.Verify(
                fu =>
                fu.CopyFileOverwrite(
        ...
        {
            var testCase = new TestCase("A.C.M", new System.Uri("executor://testExecutor"), Assembly.GetEntryAssembly().Location);
            var kvpArray = new[]
                    {
            ...
            var currentExecutingFolder = Path.GetDirectoryName(Assembly.GetEntryAssembly().Location);
