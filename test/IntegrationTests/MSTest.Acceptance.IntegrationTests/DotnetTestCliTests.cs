...
public class DotnetTestCliTests : AcceptanceTestBase<NopAssetFixture>
{
    ...
    public async Task DotnetTest_Should_Execute_Tests(string tfm, BuildConfiguration buildConfiguration)
    {
        ...
        DotnetMuxerResult compilationResult = await DotnetCli.RunAsync($"test -m:1 -nodeReuse:false --directory {generator.TargetAssetPath}", AcceptanceFixture.NuGetGlobalPackagesFolder.Path);
