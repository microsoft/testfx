    ...
    where TFixture : TestAssetFixtureBase, new()
{
    ...
    protected async Task<TestingPlatformClient> StartAsServerAndConnectToTheClientAsync(TestHost testHost)
    {
        ...
        (IProcessHandle processHandler, Task outputAndErrorTask) = ProcessFactory.Start(processConfig, cleanDefaultEnvironmentVariableIfCustomAreProvided: false);
        ...
        return new TestingPlatformClient(new(tcpClient.GetStream()), tcpClient, processHandler, outputAndErrorTask);
    }
}
