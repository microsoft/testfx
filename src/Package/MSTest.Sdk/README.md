...
The `--report-gh` option is provided by `Microsoft.Testing.Extensions.GitHubActionsReport`, which is included in the `Default` and `AllMicrosoft` profiles. Reporting remains opt-in at execution time: pass `--report-gh` to activate it when running on GitHub Actions.
...
Set `EnableMicrosoftTestingExtensionsGitHubActionsReport` to `false` to remove the extension. When included, `--help` lists `--report-gh` and its related options.
