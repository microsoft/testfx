...
#pragma warning restore CS0618, MTP0001 // Type or member is obsolete
    ...
    // Emits a workflow command ('::error' or '::warning') to the output device, prepended with a
    // newline so the command token always starts at column 0 on its own line. In CI, the terminal
    // output device runs in SimpleAnsi mode and emits a color reset ('\e[m') WITHOUT a trailing
    // newline after the preceding colored test block. Emitting the annotation directly would yield
    // "\e[m::error ..." and GitHub only recognizes a workflow command when the line begins with '::',
    ...
    private Task DisplayAnnotationLineAsync(string annotationLine, CancellationToken cancellationToken)
        => _outputDisplay.DisplayAsync(this, new FormattedTextOutputDeviceData($"\n{annotationLine}"), cancellationToken);
