...
internal abstract class CommonTestHost(ServiceProvider serviceProvider) : ITestHost
{
    ...
    {
        List<object> notifiedServices = [];
        foreach (object service in serviceProvider.Services)
        {
            // We call test session for output device in custom workflow
            if (service is IPlatformOutputDevice)
            {
                continue;
            }
            else if (service is ITestSessionLifetimeHandler testSessionLifetimeHandler)
            {
                if (notifiedServices.Contains(service))
                {
                    continue;
                }
            ...
            else if (service is BaseMessageBus messageBus)
            {
                foreach (IDataConsumer dataConsumer in messageBus.DataConsumerServices)
                {
                    // We call test session for output device in custom workflow
                    if (dataConsumer is IPlatformOutputDevice)
                    {
                        continue;
                    }
                    else if (dataConsumer is ITestSessionLifetimeHandler sessionConsumer)
                    {
                        if (notifiedServices.Contains(service))
                        {
                            continue;
                        ...
                        // We call test session for output device in custom workflow
                        if (sessionConsumer is IPlatformOutputDevice)
                        {
                            continue;
                        }

                        await sessionConsumer.OnTestSessionStartingAsync(sessionUid, cancellationToken);
                        notifiedServices.Add(sessionConsumer);
                    }
                }
            }
        }
            ...
            else if (service is BaseMessageBus messageBus)
            {
                foreach (IDataConsumer dataConsumer in messageBus.DataConsumerServices)
                {
                    ...
                    else if (dataConsumer is ITestSessionLifetimeHandler sessionConsumer)
                    {
                        ...
                        await sessionConsumer.OnTestSessionFinishingAsync(sessionUid, cancellationToken);
