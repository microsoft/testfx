...
internal sealed class AsyncConsumerDataProcessor : IAsyncConsumerDataProcessor
{
    ...
    public AsyncConsumerDataProcessor(IDataConsumer dataConsumer, ITask task, CancellationToken cancellationToken)
        : this(dataConsumer, task, cancellationToken, ShutdownTimeouts.DefaultCanceledConsumerCompletion)
    {
    }

    public AsyncConsumerDataProcessor(IDataConsumer dataConsumer, ITask task, CancellationToken cancellationToken, TimeSpan canceledShutdownTimeout)
    {
    ...
    // Completing the channel is what makes the consume loop terminate. We deliberately do not block here: the
