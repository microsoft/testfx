...
internal static class ExtensionValidationHelper
{
    ...
    public static void ValidateUniqueExtension<T>(IEnumerable<T> existingExtensions, IExtension newExtension, Func<T, IExtension> extensionSelector)
    {
        ...
        {
            var allDuplicates = duplicates.Select(extensionSelector).Concat([newExtension]).ToArray();
    ...
    /// <param name="existingExtensions">Collection of existing extensions to check against</param>
    /// <param name="newExtension">The new extension being registered</param>
    public static void ValidateUniqueExtension(IEnumerable<IExtension> existingExtensions, IExtension newExtension)
    {
        ValidateUniqueExtension(existingExtensions, newExtension, x => x);
    }
}