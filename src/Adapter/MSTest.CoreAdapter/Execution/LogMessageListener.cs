...
namespace Microsoft.VisualStudio.TestPlatform.MSTest.TestAdapter.Execution
{
    ...
    public class LogMessageListener : IDisposable
    {
        ...
        {
            var output = this.redirectedStandardOutput.ToString();
            this.redirectedStandardOutput.Clear();
            return output;
        }

        public string GetAndClearStandardError()
        {
            var output = this.redirectedStandardError.ToString();
            this.redirectedStandardError.Clear();
            return output;
        }

        public string GetAndClearDebugTrace()
        {
            var writer = traceListener?.GetWriter();
            if (writer == null)
            {
                return null;
            }
