...
#endif
{
    ...
    public AssemblyUtility(IAdapterTraceLogger logger) => _logger = logger;
    ...
    internal virtual List<string> GetSatelliteAssemblies(string assemblyPath)
    {
        ...
        {
            _logger.LogError("AssemblyUtilities.GetSatelliteAssemblies: the specified file '{0}' is not managed assembly.", assemblyPath);
        ...
        foreach (string dir in Cultures)
        {
            ...
            foreach (string extension in _assemblyExtensions)
            {
                ...
                if (File.Exists(satellitePath))
                {
                    ...
                    {
                        _logger.LogError("AssemblyUtilities.GetSatelliteAssemblies: found assembly '{0}' installed as satellite but it's not managed assembly.", satellitePath);
                        continue;
                    }
    ...
    internal virtual /* for mocking purposes - should be refactored */ IReadOnlyList<string> GetFullPathToDependentAssemblies(string assemblyPath, string? configFile, out IList<string> warnings)
    {
        ...
        _logger.LogInfo("AssemblyDependencyFinder.GetDependentAssemblies: start.");
        ...
        AppDomainUtilities.SetConfigurationFile(setupInfo, configFile, _logger);
        _logger.LogInfo("AssemblyDependencyFinder.GetDependentAssemblies: Using config file: '{0}'.", setupInfo.ConfigurationFile);
        ...
        try
        {
            ...
            _logger.LogInfo("AssemblyDependencyFinder.GetDependentAssemblies: Created AppDomain.");
...
#pragma warning restore CA1862 // Use the 'StringComparison' method overloads to perform case-insensitive string comparisons
        ...
        finally
        {
            if (appDomain != null)
            {
                _logger.LogInfo("AssemblyDependencyFinder.GetDependentAssemblies: unloading AppDomain...");
                AppDomain.Unload(appDomain);
                _logger.LogInfo("AssemblyDependencyFinder.GetDependentAssemblies: unloading AppDomain succeeded.");
            }
        }
    }
