...
internal static class MSTestTestNodeConverter
{
    ...
    {
        // Skip AddTestMethodIdentifier: in-progress nodes are "test started" signals and no consumer reads
        // TestMethodIdentifierProperty from them (IDEs and reporters already hold the full identity from the
        // discovery node). Skipping the parse avoids a ManagedNameParser.ParseManagedMethodName call and the
