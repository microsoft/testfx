...
namespace Microsoft.VisualStudio.TestPlatform.MSTestAdapter.PlatformServices
{
    ...
    public class TestDataSource : ITestDataSource
    {
        ...
        public IEnumerable<object> GetData(UTF.ITestMethod testMethodInfo, ITestContext testContext)
        {
            ...
            {
                Path.GetDirectoryName(new Uri(testMethodInfo.MethodInfo.Module.Assembly.Location).LocalPath)
            };
...
/* Unmerged change from project 'PlatformServices (net462)'
Before:
...
After:
                TestDataSource.GetConnectionProperties(testMethodInfo.GetAttributes<UTF.DataSourceAttribute>(false)[0], out providerNameInvariant, out connectionString, out tableName, out dataAccessMethod);
*/

/* Unmerged change from project 'PlatformServices (netstandard2.0)'
Before:
                this.GetConnectionProperties(testMethodInfo.GetAttributes<UTF.DataSourceAttribute>(false)[0], out providerNameInvariant, out connectionString, out tableName, out dataAccessMethod);
After:
                TestDataSource.GetConnectionProperties(testMethodInfo.GetAttributes<UTF.DataSourceAttribute>(false)[0], out providerNameInvariant, out connectionString, out tableName, out dataAccessMethod);
*/
            ...
            {
                throw new Exception("Failed to get connection properties!", ex);
            }
...
After:
                    IEnumerable<int> permutation = TestDataSource.GetPermutation(dataAccessMethod, rows.Length);
*/
