...
safe-outputs:
  ...
  threat-detection:
    prompt: >
      End the response with exactly one complete THREAT_DETECTION_RESULT JSON
      object on a single line. Do not wrap that line in Markdown, a code fence,
      emphasis markers, or any other text formatting.
    engine:
      id: copilot
