...
description: >-
  ...
  serialization that costs throughput. Submits one ranked, scope-aware,
  confidence-tagged COMMENT review when findings exist or when explicitly
  requested. Read-only and advisory.
...
(`${{ steps.extract.outputs.has_changed_tests }}` is `false`), follow the
"nothing to audit" fallback behavior in the wrapper output section and stop.
...
When this is an automatic `pull_request` run and the audit found no findings,
call `noop` and stop without publishing a review. The main expert review already
covers clean parallel-safety, so a second all-clear review would only add noise.
For slash-command runs, always publish the requested report.

Otherwise, submit **exactly one** `submit-pull-request-review` with
`event: "COMMENT"` for the resolved pull request
`${{ steps.resolve.outputs.pr_number }}`. Structure the review body as follows:
...
report only for a slash-command run. For an automatic `pull_request` run, call
`noop` and stop:
...
After the single review submission, call `noop` with a brief status such as
