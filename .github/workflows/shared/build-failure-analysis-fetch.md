      ...
      - name: Download binlogs from the failed Azure Pipelines build
        ...
        run: |
          ...
          for name in "${names[@]}"; do
            ...
            [ -z "${url}" ] && { echo "::warning::No download URL for ${safe_name}."; continue; }
            ...
            if [ "${ZIP_CAP}" -le 0 ]; then
              echo "::warning::Cumulative compressed download budget ${MAX_TOTAL_ZIP_BYTES} is exhausted before ${safe_name}; stopping downloads."; break
            fi
            ...
            if [ "${TIME_LEFT}" -le 0 ]; then
              echo "::warning::Download time budget ${FETCH_BUDGET}s exhausted before ${safe_name}; stopping downloads."; break
            fi
            ...
            if [ "${ZIP_BYTES}" -gt "${ZIP_CAP}" ]; then
              echo "::warning::Skipping ${safe_name}: download exceeded the ${ZIP_CAP}-byte cap."; continue
            fi
            ...
            if ! printf '%s' "${UNCOMP}" | grep -qE '^[0-9]+$'; then
              echo "::warning::Skipping ${safe_name}: could not determine uncompressed size (unparseable unzip output)."; continue
            fi
