...
> [!IMPORTANT]
> Never hand-edit `*.lock.yml`, `package.json`, `requirements.txt`, or `go.mod` under `.github/workflows/`. They are regenerated by `gh aw compile`.
...
- **Same applies to Dependabot updates** to `package.json` / `requirements.txt` / `go.mod` under `.github/workflows/`: never merge those PRs directly; update the source `.md` files and rerun `gh aw compile --dependabot` to bundle the fixes.
