...
The gate rejects unpinned references, missing or non-version-shaped version comments, versions that disagree with the ledger, SHAs that disagree with the ledger, one SHA labelled with conflicting versions across files, an action pinned to more than one SHA across the directory, and `gh-aw-manifest` header entries that drift from the ledger. `gh aw` records an annotated *tag object* SHA in the ledger for a few actions while emitting the dereferenced *commit* SHA in `uses:` lines; each such pair is recorded explicitly in the script's `DEREFERENCED_TAG_SHAS` table (verify a new one with `gh api repos/<owner>/<repo>/git/tags/<tag-object-sha> --jq .object.sha`) so that every *other* SHA still fails.
