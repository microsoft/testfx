...
| [`pipeline-test-triage.md`](./pipeline-test-triage.md) | Azure Pipelines `microsoft.testfx` check `completed` + manual | Collects bounded Azure DevOps test evidence and analyzes failures, retries/flakiness, crash or hang diagnostics, and historically abnormal durations. Creates deduplicated Bug issues only when recurrence and actionability thresholds are met. |
