...
| [`pipeline-test-triage.md`](./pipeline-test-triage.md) | Failed Azure Pipelines `microsoft.testfx (Build …)` checks, aggregate `microsoft.testfx` check `completed`, + manual | Posts deduplicated preliminary PR feedback from failed build legs, then analyzes the completed build for failures, retries/flakiness, crash or hang diagnostics, and historically abnormal durations. The final PR comment supersedes preliminary feedback; Bug issues are created only when recurrence and actionability thresholds are met. |
