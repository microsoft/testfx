...
- Set `<PackageDescriptionDetail>` rather than `<PackageDescription>`. The published description is composed as `$(PackageDescriptionPreamble)` + a blank line + `$(PackageDescriptionDetail)`; the preamble is the shared first paragraph, defaulted per product area in [`src/Directory.Build.props`](../src/Directory.Build.props) (MSTest) and [`src/Platform/Directory.Build.props`](../src/Platform/Directory.Build.props) (Microsoft.Testing.Platform).
- Wrap a multi-paragraph `PackageDescriptionDetail` in `<![CDATA[ ... ]]>` and keep its continuation lines unindented. MSBuild trims a property value but not the indentation of its inner lines, so an inline indented description leaks the .csproj whitespace into the published text.
