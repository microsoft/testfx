...
3. **Bare-token property reference in conditions** — A condition like `'(Foo)' != ''` or `'(TestingPlatformCommandLineArguments)' != ''` is **not** a property reference — it compares the literal string `(Foo)` (or `(TestingPlatformCommandLineArguments)`) to the empty string and is therefore always true. Property references in conditions MUST use the `$(Name)` form: `'$(Foo)' != ''`. Same defect class for items (`@(Name)`) and metadata (`%(Name)`). Trigger this check whenever you see a quoted token starting with `(` immediately after the opening quote. **Severity: 🔴 Error.**
