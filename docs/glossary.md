...
When `UseUwp=true`, `UseVSTest` defaults to `true` because VSTest is the established host for AppContainer test applications, but MTP is supported: set `UseVSTest=false` to opt into it. WinUI keeps the MTP default; set `UseVSTest=true` only for a packaged WinUI test application.
